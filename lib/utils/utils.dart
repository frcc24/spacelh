...
class Utils {
  ...
  static Vector2 generateRandomVelocity(Vector2 screenSize, int min, int max) {
    ...
    while (result == Vector2.zero()) {
      result =
          Vector2((randomGenerator.nextInt(3) - 1) * randomGenerator.nextDouble(), (randomGenerator.nextInt(3) - 1) * randomGenerator.nextDouble());
    }
    result.normalize();
  ...
  static Vector2 wrapPosition(Vector2 bounds, Vector2 position) {
    ...
    if (position.x >= bounds.x) {
      result.x = bounds.x - 10;
    } else if (position.x <= 0) {
      result.x = 10;
    }
    if (position.y >= bounds.y) {
      result.y = bounds.y - 10;
