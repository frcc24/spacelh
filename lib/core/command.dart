...
// import 'package:spacelh/components/spaceship.dart';
...
class Broker {
  ...
  void addCommand(Command command) {
    ...
    if (command.mustBeUnique()) {
      if (_duplicatesWatcher.any((element) => element.getId() == command.getId())) {
...
class UserTapUpCommand extends Command {
  ...
  @override
  void execute() {
    debugPrint('<UserTapUpCommand> player is alive: ${_getController().contains(player)}');
...
class BulletFiredCommand extends Command {
  ...
  void execute() {
    ...
    BulletBuildContext context = BulletBuildContext()
      ...
      ..position = _getController().getSpaceship().getMuzzleComponent.absolutePosition
      ..velocity = velocity
      ..size = Vector2(4, 4);
...
class BulletDestroyCommand extends Command {
  ...
  void execute() {
    ...
    if (_getController().children.any((element) => targetBullet == element)) {
      debugPrint('<BulletDestroyCommand> removing id: ${targetBullet.hashCode}');
...
//   @override
//   void execute() async {
//     if (_getController().getScoreBoard.getScore > _getController().getScoreBoard.getHighScore) {
...
//     if (_getController().currLevelObjectStack.any((element) => targetBonus == element)) {
...
//       UpdateScoreboardScoreCommand(_getController().getScoreBoard).addToController(_getController());
//     }
