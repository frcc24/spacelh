...
class Bullet03 extends Bullet {
  static const double defaultSpeed = 700.00;
  static final Vector2 defaultSize = Vector2.all(16.00);

  Bullet03(Vector2 position, Vector2 velocity)
  ...
  Bullet03.fullInit(Vector2 position, Vector2 velocity, Vector2? size, double? speed, int? health, int? damage)
      : super.fullInit(position, velocity, size: size, speed: speed, health: health, damage: damage);

  @override
  Future<void> onLoad() async {
    await super.onLoad();
    spriteSize = Vector2(64.0, 128.0);

    spriteSheet = SpriteSheet(
      image: await gameRef.images.load('bullet03.png'),
      srcSize: Vector2(16.0, 64.0),
    );

    animationBullet = spriteSheet.createAnimation(row: 0, stepTime: 0.1, from: 0, to: 3);
